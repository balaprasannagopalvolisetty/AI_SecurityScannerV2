  ...
  useEffect(() => {
    const loadScans = async () => {
      try {
        ...
        // Calculate stats
        const completed = data.filter(scan => scan.status === 'completed').length;
        const pending = data.filter(scan => scan.status === 'pending' || scan.status === 'scanning').length;
