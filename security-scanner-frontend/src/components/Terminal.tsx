...
const Terminal: React.FC = () => {
  ...
  const handleCommand = () => {
    ...
    // Fix: Improved command handling with better parsing
...
exit                 - Exit terminal
      ...
      case "scan":
        if (args.length < 2) {
          ...
          newOutputs.push({ text: `Starting scan for ${formattedUrl}...`, type: "info" });
...
Use 'status ${scanId}' to check the scan status`,
      ...
      // Handle URLs directly (common user mistake)
      default:
        if (command.includes("http") || command.includes(".com") || command.includes(".org") || command.includes(".net")) {
          // Treat as a scan command if it looks like a URL
          newOutputs.push({ text: `Starting scan for ${command}...`, type: "info" });
...
Use 'status ${scanId}' to check the scan status`,
          ...
          newOutputs.push({ text: `Command not found: ${mainCommand}. Type 'help' to see available commands.`, type: "error" });
        }
    }
